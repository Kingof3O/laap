...
export default function App() {
  ...
  const runtimeLabel = useMemo(() => processes.game ? 'League running' : processes.league_client ? 'Authenticated' : processes.riot_client || launchRequested ? 'Waiting for Riot login' : sessionId ? 'Lease active' : 'Idle', [launchRequested, processes, sessionId])
  ...
  const release = async () => { if (!sessionId) return; await apiRequest(`/api/leases/${sessionId}/release`, { method: 'POST', body: JSON.stringify({ reason: 'manual' }) }).catch((cause) => setError(cause instanceof Error ? cause.message : String(cause))); setSessionId(null); setLaunchRequested(false) }
  const logout = async () => { if (sessionId) await apiRequest(`/api/leases/${sessionId}/release`, { method: 'POST', body: JSON.stringify({ reason: 'logout' }) }).catch(() => undefined); await apiRequest('/api/auth/logout', { method: 'POST' }).catch(() => undefined); setUser(null); setAccounts([]); setDeviceId(null); setSessionId(null); setLaunchRequested(false) }
