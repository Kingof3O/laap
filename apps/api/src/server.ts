...
export async function createApp(overrides: Partial<RuntimeConfig> & { dataDir?: string } = {}): Promise<AppHandle> {
  ...
  const server = http.createServer(async (request, response) => {
    ...
    try {
      await route(request, response, service, runtime, loginLimiter)
  ...
  reaper?.unref()
  return { server, database, service, close: async () => { if (reaper) clearInterval(reaper); await database.save(); database.close(); if (!server.listening) return; await new Promise<void>((resolve, reject) => server.close((error) => error ? reject(error) : resolve())) } }
}
...
export async function createSupabaseApp(overrides: Partial<RuntimeConfig> = {}): Promise<AppHandle> {
  ...
  reaper?.unref()
  return { server, service, close: async () => { if (reaper) clearInterval(reaper); if (!server.listening) return; await new Promise<void>((resolve, reject) => server.close((error) => error ? reject(error) : resolve())) } }
}
