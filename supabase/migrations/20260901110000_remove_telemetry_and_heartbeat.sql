-- Migration: Remove heartbeat and runtime telemetry from account_sessions
-- Replaces aggressive 90-second heartbeat check with generous 4-hour max lease expiry
...
begin
  ...
  if not exists (select 1 from public.account_assignments where account_id = p_account_id and user_id = p_user_id and status = 'active' and (expires_at is null or expires_at > now())) then
    return jsonb_build_object('success', false, 'code', 'NO_ACTIVE_ASSIGNMENT');
  end if;
...
-- Heartbeat no-op stubs for backward compatibility
create or replace function public.heartbeat_account_session(p_session_id uuid, p_runtime_state text default null)
returns jsonb
language plpgsql
security definer
set search_path = public, auth
...
create or replace function public.heartbeat_account_session_for_user(p_user_id uuid, p_session_id uuid, p_runtime_state text default null)
returns jsonb
language plpgsql
security definer
set search_path = pg_catalog, public, auth
